...
import { IAuditVisitReport } from "./WorkBookSearch";
export class AuditReportPdfService {
    ...
    public static generate(report: IAuditVisitReport) {
        ...
        if (report.infringementsAndComments.length != 0) {
            ...
            autoTable(doc, {
                ...
                head: [
                    [
                        { content: "Infractions/Remarques", styles: { lineWidth: { right: 0.2 }, lineColor: [0, 0, 0], fontStyle: "bold", textColor: [255, 255, 255], halign: "center", fontSize: 14 } },
    ...
    private static formatRowPropriety(title: string, value: string | null, topLine = false): RowInput {
        if (topLine)
            return [
                { content: title ?? "-", styles: { lineColor: [255, 255, 255], lineWidth: { bottom: 0.1, top: 0.1 }, fillColor: [31, 41, 55], fontStyle: "bold", textColor: [255, 255, 255], cellWidth: 50, halign: "center", fontSize: 14 } },
                { content: value ?? "-", styles: { lineColor: [0, 0, 0], lineWidth: { bottom: 0.1, top: 0.1 } } },
            ];
        return [
            { content: title ?? "-", styles: { lineColor: [255, 255, 255], lineWidth: { bottom: 0.1 }, fillColor: [31, 41, 55], fontStyle: "bold", textColor: [255, 255, 255], cellWidth: 50, halign: "center", fontSize: 14 } },
            { content: value ?? "-", styles: { lineColor: [0, 0, 0], lineWidth: { bottom: 0.1 } } },
        ];
    }
    ...
    private static formatInfringementsAndComments(report: IAuditVisitReport): RowInput[] {
        return report.infringementsAndComments.map((inf) => {
            return [
                { content: inf.infringement, styles: { lineColor: [31, 41, 55], lineWidth: { right: 0.2 } } },
